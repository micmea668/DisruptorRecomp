...
| In-game settings/dev menu | Implemented; live validation pending | Host-only Dear ImGui overlay changes safe controls and presentation features live, blocks guest input while open, suspends interpolation independently, and exposes read-only geometry/interpolation diagnostics. Windows x64 build and all 11 CTests pass |
