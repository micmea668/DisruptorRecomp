...
struct CodeGenConfig {
    ...
    // Manual screen-window counterparts for zero-bound branches, constant /
    // zero-store clamps, right-edge constants, and packed [left,right] spans.
    // Each configured site is instruction-shape checked by codegen.
    std::set<uint32_t> ws_cull_lower_branch_sites;
    std::set<uint32_t> ws_cull_lower_value_sites;
    std::set<uint32_t> ws_cull_upper_value_sites;
    std::set<uint32_t> ws_cull_span_value_sites;
    ...
    // Exact `addi[u] rt,rs,+/-half_fov` wrapping-yaw sites. The runtime widens
    // only the signed offset in the site's declared angular unit system.
    std::vector<PSXRecompV4::WidescreenYawConeSite> ws_cull_yaw_cone_sites;

    // Exact signed `slt dst,bound,horizontal` side-plane comparisons. The
    // horizontal operand is projection-scaled only while widescreen is live.
