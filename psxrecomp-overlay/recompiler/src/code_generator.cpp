...
std::string CodeGenerator::generate_branch_condition(uint32_t instr, uint32_t addr) {
    ...
    // Explicit manual-screen lower-edge branches. Disruptor-class portal and
    // wall renderers project in CPU code and use zero-bound branches rather
    // than the usual SLTI/BLTZ funnel. Move that zero edge to -x_margin while
    // retaining the original branch sense. At margin 0 these expressions are
    // exactly the vanilla predicates.
    if (config_.ws_cull_lower_branch_sites.count(addr)) {
        if (opcode == 0x01) {
            const uint32_t regimm_op = (instr >> 16) & 0x1Fu;
            if (regimm_op == 0x00u)
                return fmt::format("(int32_t){} < -psx_ws_x_margin()"
                                   " /* ws manual lower edge */", reg_name(rs));
            if (regimm_op == 0x01u)
                return fmt::format("(int32_t){} >= -psx_ws_x_margin()"
                                   " /* ws manual lower edge */", reg_name(rs));
        }
        if (opcode == 0x06u || opcode == 0x16u)
            return fmt::format("(int32_t){} <= -psx_ws_x_margin()"
                               " /* ws manual lower edge */", reg_name(rs));
        if (opcode == 0x07u || opcode == 0x17u)
            return fmt::format("(int32_t){} > -psx_ws_x_margin()"
                               " /* ws manual lower edge */", reg_name(rs));
        if (!config_.overlay_mode) {
            fmt::print(stderr,
                       "ERROR: [widescreen.cull] lower_branch site 0x{:08X} "
                       "is not bltz/bgez/blez/bgtz (0x{:08X})\n",
                       addr, instr);
            std::exit(1);
        }
    }
...
std::string CodeGenerator::translate_instruction(uint32_t addr, uint32_t instr) {
    ...
    // Full-word-guarded wrapping-yaw half-FOV offsets. Disruptor, for example,
    // seeds portal traversal with yaw +/-32 in an eight-bit turn. Widen the
    // signed offset geometrically while leaving the game's base yaw and wrap
    // behavior untouched.
    for (const auto& site : config_.ws_cull_yaw_cone_sites) {
        if ((site.address & 0x1FFFFFFFu) !=
            (addr & 0x1FFFFFFFu)) continue;
        if (instr != site.expected) {
            if (config_.overlay_mode) continue;
            fmt::print(stderr,
                       "ERROR: yaw cone expected 0x{:08X} at 0x{:08X}, "
                       "found 0x{:08X}\n",
                       site.expected, addr, instr);
            std::exit(1);
        }
        if ((opcode != 0x08u && opcode != 0x09u) ||
            get_rs(instr) == 0u || site.full_turn < 16u) {
            fmt::print(stderr,
                       "ERROR: yaw cone site 0x{:08X} is not a valid "
                       "ADDI/ADDIU rt,rs,offset (0x{:08X})\n",
                       addr, instr);
            std::exit(1);
        }
        const uint32_t dst = get_rt(instr);
        const uint32_t src = get_rs(instr);
        const int32_t vanilla = (int16_t)(instr & 0xFFFFu);
        return fmt::format(
            "{} = {} + (uint32_t)psx_ws_yaw_cone_offset({}, {}u);"
            "  /* aspect-scaled wrapping yaw half-FOV */{}",
            reg_name(dst), reg_name(src), vanilla, site.full_turn, comment);
    }

    // Symmetric camera-horizontal side-plane rejects. Scaling the horizontal
    // operand is the exact counterpart of widening the projection; vertical
    // tests in the same functions remain completely vanilla.
    for (const auto& site : config_.ws_cull_horizontal_slt_sites) {
        if ((site.address & 0x1FFFFFFFu) !=
            (addr & 0x1FFFFFFFu)) continue;
        if (instr != site.expected) {
            if (config_.overlay_mode) continue;
            fmt::print(stderr,
                       "ERROR: horizontal SLT expected 0x{:08X} at "
                       "0x{:08X}, found 0x{:08X}\n",
                       site.expected, addr, instr);
            std::exit(1);
        }
        if (opcode != 0u || funct != 0x2Au ||
            ((instr >> 6) & 31u) != 0u) {
            fmt::print(stderr,
                       "ERROR: horizontal SLT site 0x{:08X} is not signed "
                       "register-register SLT (0x{:08X})\n",
                       addr, instr);
            std::exit(1);
        }
        return fmt::format(
            "{} = psx_ws_horizontal_slt({}, {});"
            "  /* aspect-scaled camera-horizontal side plane */{}",
    ...
    // Manual screen-window bound materialization. These are deliberately
    // exact-site and instruction-shape guarded: a generic 320 constant can be
    // a centre-relative divider, texture width, or packed span, and those
    // meanings are not interchangeable.
    const bool ws_lower_value = config_.ws_cull_lower_value_sites.count(addr);
    const bool ws_upper_value = config_.ws_cull_upper_value_sites.count(addr);
    if (ws_lower_value || ws_upper_value) {
        const char* helper = ws_lower_value
            ? "psx_ws_cull_lower_value" : "psx_ws_cull_upper_value";
        const char* tag = ws_lower_value ? "lower" : "upper";
        if ((opcode == 0x08u || opcode == 0x09u || opcode == 0x0Du) &&
            get_rs(instr) == 0u) {
            const uint32_t dst = get_rt(instr);
            const int32_t vanilla = opcode == 0x0Du
                ? (int32_t)get_imm16_u(instr) : (int32_t)get_imm16(instr);
            return fmt::format(
                "{} = (uint32_t){}({});"
                "  /* ws manual {} bound value */{}",
                reg_name(dst), helper, vanilla, tag, comment);
        }
        if (ws_lower_value && opcode == 0x00u &&
            (funct == 0x21u || funct == 0x25u) &&
            get_rs(instr) == 0u && get_rt(instr) == 0u) {
            return fmt::format(
                "{} = (uint32_t)psx_ws_cull_lower_value(0);"
                "  /* ws manual lower clamp */{}",
                reg_name(get_rd(instr)), comment);
        }
        if (ws_lower_value && opcode == 0x2Bu && get_rt(instr) == 0u) {
            const uint32_t base = get_rs(instr);
            const int16_t offset = get_imm16(instr);
            const std::string store_addr = offset == 0
                ? reg_name(base)
                : fmt::format("{} + {}", reg_name(base), (int)offset);
            return fmt::format(
                "g_debug_last_store_pc = 0x{:08X}u; "
                "psx_store_cycle_barrier(); cpu->write_word({}, "
                "(uint32_t)psx_ws_cull_lower_value(0));"
                "  /* ws manual lower clamp store */{}",
                addr, store_addr, comment);
        }
        if (!config_.overlay_mode) {
            fmt::print(stderr,
                       "ERROR: [widescreen.cull] {}_value site 0x{:08X} "
                       "is not a supported constant/clamp form (0x{:08X})\n",
                       tag, addr, instr);
            std::exit(1);
        }
    }
    if (config_.ws_cull_span_value_sites.count(addr)) {
        if ((opcode == 0x08u || opcode == 0x09u || opcode == 0x0Du) &&
            get_rs(instr) == 0u) {
            const uint32_t dst = get_rt(instr);
            const uint32_t vanilla = opcode == 0x0Du
                ? (uint32_t)get_imm16_u(instr)
                : (uint32_t)(int32_t)get_imm16(instr);
            return fmt::format(
                "{} = psx_ws_cull_span_value(0x{:08X}u);"
                "  /* ws manual packed [left,right] span */{}",
                reg_name(dst), vanilla, comment);
        }
        if (!config_.overlay_mode) {
            fmt::print(stderr,
                       "ERROR: [widescreen.cull] span_value site 0x{:08X} "
                       "is not ADDI/ADDIU/ORI rt,zero,imm (0x{:08X})\n",
                       addr, instr);
            std::exit(1);
        }
    }
    ...
    // Widescreen CPU-projected screen-X correction
    // ([widescreen.backdrop] x_sites). Most sites are `sh rt,off(base)` stores
    // from parallax layers. Some 3D engines also calculate their portal or
    // billboard projection wholly on the CPU and materialize the final X with
    // `addiu rt,rs,centre`; route those exact sites through the same classic
    // projection squash as the GTE. This keeps the guest's original 0..W clip
    // invariants intact instead of widening portal spans into negative values.
    // Both helpers are identities at 4:3. Instruction shape is verified so a
    // stale game address fails loudly rather than changing unrelated code.
    if (config_.ws_backdrop_x_sites.count(addr)) {
        ...
        } else if (opcode == 0x09) {  // addiu rt,rs,screen-centre
            uint32_t rs = get_rs(instr), rt = get_rt(instr);
            int16_t imm = get_imm16(instr);
            return fmt::format(
                "{} = (uint32_t)psx_ws_project_x((int32_t)({} + (uint32_t){}));"
                "  /* ws CPU-projected screen-x */{}",
                reg_name(rt), reg_name(rs), (int32_t)imm, comment);
        } else if (!config_.overlay_mode) {
            fmt::print(stderr, "ERROR: [widescreen.backdrop] x site 0x{:08X} is not "
                       "sh or addiu (opcode 0x{:02X})\n", addr, opcode);
            std::exit(1);
        }
        // overlay variant: addr is different code here — fall through to vanilla.
    }
...
void CodeGenerator::emit_runtime_externs(std::ostream& ss) const {
    ...
    ss << "extern int32_t psx_ws_cull_lower_value(int32_t vanilla);  /* ws manual lower bound (gpu.c) */\n";
    ss << "extern int32_t psx_ws_cull_upper_value(int32_t vanilla);  /* ws manual upper bound (gpu.c) */\n";
    ss << "extern uint32_t psx_ws_cull_span_value(uint32_t vanilla); /* ws packed [left,right] span (gpu.c) */\n";
    ...
    ss << "extern int32_t psx_ws_yaw_cone_offset(int32_t vanilla, uint32_t full_turn); /* aspect-scaled wrapping yaw half-FOV */\n";
    ss << "extern uint32_t psx_ws_horizontal_slt(uint32_t bound, uint32_t horizontal); /* aspect-scaled signed horizontal side plane */\n";
    ss << "extern int  psx_ws_backdrop_x(int x);  /* widescreen backdrop screenX squash (gpu.c) */\n";
    ss << "extern int  psx_ws_project_x(int x);   /* classic-wide CPU projection squash (gpu.c) */\n";
