...
uint32_t overlay_codegen_config_hash(const GameConfig& c) {
    ...
    h.words("cull_lower_branch", c.ws_cull_lower_branch_sites);
    h.words("cull_lower_value", c.ws_cull_lower_value_sites);
    h.words("cull_upper_value", c.ws_cull_upper_value_sites);
    h.words("cull_span_value", c.ws_cull_span_value_sites);
    ...
    std::vector<WidescreenYawConeSite> yaw_cone_sites =
        c.ws_cull_yaw_cone_sites;
    std::sort(yaw_cone_sites.begin(), yaw_cone_sites.end(),
              [](const auto& a, const auto& b) {
                  if (a.address != b.address) return a.address < b.address;
                  if (a.expected != b.expected) return a.expected < b.expected;
                  return a.full_turn < b.full_turn;
              });
    h.tag("cull_yaw_cone");
    h.u32((uint32_t)yaw_cone_sites.size());
    for (const auto& site : yaw_cone_sites) {
        h.u32(site.address);
        h.u32(site.expected);
        h.u32(site.full_turn);
    }

    std::vector<WidescreenHorizontalSltSite> horizontal_slt_sites =
        c.ws_cull_horizontal_slt_sites;
    std::sort(horizontal_slt_sites.begin(), horizontal_slt_sites.end(),
              [](const auto& a, const auto& b) {
                  if (a.address != b.address) return a.address < b.address;
                  return a.expected < b.expected;
              });
    h.tag("cull_horizontal_slt");
    h.u32((uint32_t)horizontal_slt_sites.size());
    for (const auto& site : horizontal_slt_sites) {
        h.u32(site.address);
        h.u32(site.expected);
    }
...
GameConfig load_game_config(const fs::path& config_path_in) {
    ...
    std::vector<WidescreenHorizontalSltSite>
        ws_cull_horizontal_slt_sites;
    ...
    if (cfg.contains("widescreen")) {
        ...
        if (ws.contains("cull")) {
            ...
            load_sites("lower_branch_sites", ws_cull_lower_branch_sites);
            load_sites("lower_value_sites", ws_cull_lower_value_sites);
            load_sites("upper_value_sites", ws_cull_upper_value_sites);
            load_sites("span_value_sites", ws_cull_span_value_sites);
            ...
            if (cull.contains("yaw_cone")) {
                std::set<uint32_t> seen;
                for (const auto& item :
                     toml::find<toml::array>(cull, "yaw_cone")) {
                    WidescreenYawConeSite site;
                    site.address = parse_hex(
                        toml::find<std::string>(item, "address"),
                        "widescreen.cull.yaw_cone.address");
                    site.expected = parse_hex(
                        toml::find<std::string>(item, "expected"),
                        "widescreen.cull.yaw_cone.expected");
                    const int64_t full_turn =
                        toml::find<int64_t>(item, "full_turn");
                    if (full_turn < 16 || full_turn > 65536 ||
                        (full_turn & 3) != 0) {
                        throw std::runtime_error(fmt::format(
                            "{}: [[widescreen.cull.yaw_cone]] full_turn "
                            "must be a multiple of four in [16, 65536]",
                            config_path.string()));
                    }
                    site.full_turn = (uint32_t)full_turn;
                    const uint32_t op = site.expected >> 26;
                    const uint32_t rs = (site.expected >> 21) & 31u;
                    const int32_t imm =
                        (int16_t)(site.expected & 0xFFFFu);
                    const int32_t magnitude = imm < 0 ? -imm : imm;
                    if ((op != 0x08u && op != 0x09u) || rs == 0u ||
                        imm == 0 ||
                        magnitude >= (int32_t)(site.full_turn / 4u)) {
                        throw std::runtime_error(fmt::format(
                            "{}: [[widescreen.cull.yaw_cone]] expected must "
                            "be ADDI/ADDIU rt,rs,+/-half_fov with rs != zero "
                            "and |half_fov| < full_turn/4",
                            config_path.string()));
                    }
                    if ((site.address & 3u) != 0)
                        throw std::runtime_error(fmt::format(
                            "{}: yaw-cone address 0x{:08X} is not "
                            "instruction-aligned",
                            config_path.string(), site.address));
                    if (!seen.insert(site.address & 0x1FFFFFFFu).second)
                        throw std::runtime_error(fmt::format(
                            "{}: duplicate yaw-cone address 0x{:08X}",
                            config_path.string(), site.address));
                    ws_cull_yaw_cone_sites.push_back(site);
                }
            }
            if (cull.contains("horizontal_slt")) {
                std::set<uint32_t> seen;
                for (const auto& item :
                     toml::find<toml::array>(cull, "horizontal_slt")) {
                    WidescreenHorizontalSltSite site;
                    site.address = parse_hex(
                        toml::find<std::string>(item, "address"),
                        "widescreen.cull.horizontal_slt.address");
                    site.expected = parse_hex(
                        toml::find<std::string>(item, "expected"),
                        "widescreen.cull.horizontal_slt.expected");
                    const uint32_t op = site.expected >> 26;
                    const uint32_t fn = site.expected & 0x3Fu;
                    const uint32_t shamt = (site.expected >> 6) & 31u;
                    if (op != 0u || fn != 0x2Au || shamt != 0u) {
                        throw std::runtime_error(fmt::format(
                            "{}: [[widescreen.cull.horizontal_slt]] expected "
                            "must be a signed register-register SLT",
                            config_path.string()));
                    }
                    if ((site.address & 3u) != 0)
                        throw std::runtime_error(fmt::format(
                            "{}: horizontal-SLT address 0x{:08X} is not "
                            "instruction-aligned",
                            config_path.string(), site.address));
                    if (!seen.insert(site.address & 0x1FFFFFFFu).second)
                        throw std::runtime_error(fmt::format(
                            "{}: duplicate horizontal-SLT address 0x{:08X}",
                            config_path.string(), site.address));
                    ws_cull_horizontal_slt_sites.push_back(site);
                }
            }
    ...
    return GameConfig{
        ...
        /*ws_cull_lower_branch_sites*/ ws_cull_lower_branch_sites,
        /*ws_cull_lower_value_sites*/ ws_cull_lower_value_sites,
        /*ws_cull_upper_value_sites*/ ws_cull_upper_value_sites,
        /*ws_cull_span_value_sites*/ ws_cull_span_value_sites,
        ...
        /*ws_cull_yaw_cone_sites*/ ws_cull_yaw_cone_sites,
        /*ws_cull_horizontal_slt_sites*/ ws_cull_horizontal_slt_sites,
