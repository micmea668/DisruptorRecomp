...
static uint32_t kbless_last_mismatch[6] = {0};
...
int psx_kernel_bless_dispatchable(uint32_t phys) {
    ...
    kbless_mismatches++;
    kbless_last_mismatch[0] = phys;
    kbless_last_mismatch[1] = b->body_lo;
    kbless_last_mismatch[2] = b->body_hi;
    {
        const uint8_t *live = ram + b->body_lo;
        const uint8_t *ref = bios_rom + s_kb_rom_off +
                             (b->body_lo - s_kb_lo);
        uint32_t len = b->body_hi - b->body_lo;
        uint32_t off = 0;
        while (off < len && live[off] == ref[off]) off++;
        kbless_last_mismatch[3] = b->body_lo + off;
        kbless_last_mismatch[4] = off < len ? live[off] : 0u;
        kbless_last_mismatch[5] = off < len ? ref[off] : 0u;
    }
    return 0;
}
...
void psx_kernel_bless_mismatch_detail(uint64_t *count, uint32_t out[6]) {
    if (count) *count = kbless_mismatches;
    if (out) memcpy(out, kbless_last_mismatch, sizeof(kbless_last_mismatch));
}
