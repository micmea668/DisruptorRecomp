...
/* A diagnostic launcher can isolate the two discovered frustum layers without
 * rebuilding: "off" keeps both vanilla, "yaw" widens only portal traversal,
 * and the default "full" also widens the later horizontal side planes. */
static int ws_frustum_mode(void) {
    static int mode = -1;
    if (mode >= 0) return mode;
    const char *value = getenv("PSX_WS_FRUSTUM_MODE");
    if (value && (strcmp(value, "off") == 0 || strcmp(value, "0") == 0))
        mode = 0;
    else if (value && (strcmp(value, "yaw") == 0 || strcmp(value, "1") == 0))
        mode = 1;
    else
        mode = 2;
    printf("psxrecomp: widescreen frustum mode %s\n",
           mode == 0 ? "off" : (mode == 1 ? "yaw-only" : "full"));
    return mode;
}

int32_t psx_ws_yaw_cone_offset(int32_t vanilla, uint32_t full_turn) {
    if (ws_frustum_mode() == 0) return vanilla;
    return psx_ws_widen_angle_units(
        vanilla, full_turn, psx_ws_x_margin());
}

uint32_t psx_ws_horizontal_slt(uint32_t bound, uint32_t horizontal) {
    if (ws_frustum_mode() < 2)
        return (int32_t)bound < (int32_t)horizontal ? 1u : 0u;
    const int32_t scaled_horizontal = psx_ws_scale_horizontal_320(
        (int32_t)horizontal, psx_ws_x_margin());
    return (int32_t)bound < scaled_horizontal ? 1u : 0u;
}
...
/* Manual CPU-projected screen-window constants. These complement the compare
 * helpers above for games which clamp and queue signed [left,right] spans in
 * ordinary MIPS code. All three are exact identities when margin==0. */
int32_t psx_ws_cull_lower_value(int32_t vanilla) {
    return vanilla - psx_ws_x_margin();
}
...
uint32_t psx_ws_cull_span_value(uint32_t vanilla) {
    ...
    int32_t left = (int32_t)(int16_t)(uint16_t)(vanilla >> 16);
    uint32_t right = vanilla & 0xFFFFu;
    return ((uint32_t)(uint16_t)(left - margin) << 16) |
           (uint32_t)(uint16_t)((int32_t)right + margin);
}
...
/* Classic-wide counterpart for engines which calculate perspective screen X
 * in ordinary CPU code instead of the GTE. Exact recompiler sites call this
 * immediately after adding the projection centre. Squashing here keeps the
 * game's original non-negative 0..display-width portal spans and clip logic
 * valid; the classic present stretch restores native proportions afterward.
 * Native-wide and 4:3 are exact identities. Use 64-bit intermediates because
 * a near-plane projection may be far outside the eventual screen clamp. */
int psx_ws_project_x(int x) {
    if (ws_mode != 1 || !ws_configured()) return x;
    int64_t cx = (int64_t)ws_disp_w() / 2;
    int64_t d = (int64_t)(int32_t)x - cx;
    int64_t n = d * (int64_t)ws_xnum;
    n += d >= 0 ? ws_xden / 2 : -ws_xden / 2;
...
static void ws_nw_sync_target(void) {
    ...
    /* Geometry correction deliberately reuses the renderer's independent wide
     * surface even in classic squash mode. With zero side offset it becomes a
     * same-width presentation mirror: canonical VRAM remains integer-exact,
     * while the mirror may consume subpixel vertices and fractional yaw. */
    gr_wide_configure(native_wide ? (int)ws_disp_w() + ws_nw_extra()
                                  : (int)ws_disp_w(),
                      native_wide ? ws_nw_offset() : 0);
...
void gpu_geometry_correction_stats(uint64_t *world_triangles,
    ...
    if (world_triangles) *world_triangles = ws_geometry_world_triangles;
...
/* Queue one triangle's one-shot presentation metadata. Fractional yaw uses the
 * same exact-provenance gate as subpixel positions, so CPU-built 2D polygons
 * and incomplete world packets remain completely canonical. */
static void queue_precise_triangle(int exact,
                                   const int32_t *fx, const int32_t *fy,
                                   int a, int b, int c) {
...
static void gp0_exec_mono_tri(void) {
    ...
    prepare_precise_triangle(1, 2, 3, vx, vy);
