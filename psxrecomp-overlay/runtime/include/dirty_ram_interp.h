...
/* Detail for the most recent kernel-body validation failure. `count` is the
 * cumulative number of failed body validations; out[] is entry, body_lo,
 * body_hi, first_mismatch, live_byte, reference_byte. */
void     psx_kernel_bless_mismatch_detail(uint64_t *count, uint32_t out[6]);

/* Opt-in, low-overhead interpreter hot-PC sampler used by
 * PSX_RUNTIME_PERF_DIAG. It samples one retired instruction per `stride` and
 * returns/resets the three hottest physical PCs since the previous take. */
#define DIRTY_RAM_PERF_HOT_COUNT 3u
