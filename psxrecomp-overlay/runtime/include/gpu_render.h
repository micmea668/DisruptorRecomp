...
/* Optional presentation-only geometry correction. The authoritative PSX
 * framebuffer is always rasterized from the integer GP0 coordinates. A
 * backend may use the precise 16.16 coordinates and fractional camera yaw
 * only while mirroring the primitive into a separate display surface. */
void gr_set_geometry_correction(int enabled);
void gr_set_world_triangle(int world);
...
typedef struct GpuRenderBackend {
    ...
    void (*set_geometry_correction)(int enabled);
    void (*set_world_triangle)(int world);
